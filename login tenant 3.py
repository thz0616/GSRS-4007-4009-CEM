...
image = Image.open(r"C:\Users\tanho\PycharmProjects\all project 2\malaysia_skyscrapers_houses_megapolis_night_kuala_lumpur_cities_1920x10801.jpg")
...
def registerWindow():
    ...
    faceUploadButton = customtkinter.CTkButton(registerFrame, text="Upload",
                                               command=lambda: upload_file(faceImageEntry))
    faceUploadButton.place(x=350, y=570)

    # Add OTP Label, Entry, and Request Button
    otpLabel = customtkinter.CTkLabel(registerFrame, text="OTP: ", font=("Verdana", 25, "bold"))
    otpLabel.place(x=60, y=610)

    otpEntry = customtkinter.CTkEntry(registerFrame, font=("Verdana", 25), width=200)
    otpEntry.place(x=350, y=610)

    def request_otp():
        # Here you would implement the OTP request logic
        # For now, we'll just print a message
        print("OTP requested")
        # In a real implementation, you would generate an OTP and send it via email
        # You can use the code from gmail_otp.py for this purpose

    otpRequestButton = customtkinter.CTkButton(registerFrame, text="Request OTP", 
                                               command=request_otp, 
                                               font=("Verdana", 20))
    otpRequestButton.place(x=560, y=610)

    # Adjust the position of the register button and switch login button
    ...
    registerButton.place(x=250, y=670)
    ...
    switchLoginButton.place(x=350, y=770, anchor=CENTER)
